...
### SSR routing bridge (Packet 15)

`src/aws/lambda-handler.ts` now serves both halves of the API Gateway `$default` route. `/api` and `/api/*` keep the exact existing behavior (`createConfiguredApiRouter` -> Fetch request/response conversion). Every other path is a browser route and is delegated to the packaged Nitro SSR server, which `scripts/build-lambda.mjs` already emits with the `aws-lambda` preset into `.output-aws/server` and copies to `dist-lambda/server`. Nitro is loaded lazily on the first non-API request through a runtime specifier (`new URL("./server/index.mjs", import.meta.url).href`) so esbuild does not try to resolve a file that is copied in after the handler bundle is written. The entry point is built by the exported `createLambdaHandler(loadSsrServer)` factory, which keeps the routing decision testable without a packaged Nitro bundle, and `handler` is its production instance. A hand-built event without a headers map is normalized to `{ headers: {} }` because Nitro reads `event.headers.host` directly. No AWS resource, API contract, async worker, investigation, Bedrock, DynamoDB or IAM behavior changed.

### Static asset serving inside the Lambda package (Packet 16)

The `aws-lambda` Nitro preset registers no static handler (its `serveStatic` option is unset, so the middleware that serves public assets is never added), and `scripts/build-lambda.mjs` copies only `.output-aws/server` into `dist-lambda`. The client assets built into `.output-aws/public` were therefore absent from the package, so `/assets/*` fell through to the SSR renderer and the deployed page loaded unstyled while `/` still returned 200 HTML. `vite.config.aws.ts` now sets `serveStatic: "inline"`, one of the modes Nitro supports (`boolean | "node" | "deno" | "inline"`), which embeds the public assets into the server bundle and registers the static middleware ahead of the renderer. No handler, routing, packaging or AWS change was needed: the same packaged Nitro handler now answers the SSR document, `/assets/*`, `/favicon.ico` and `/robots.txt`, and `/api` and `/api/*` still return before the SSR server is loaded.

### Real authentication and per-user ownership (Packet 17)

Authentication is now real at the application boundary and no identity is ever read from the request. `src/auth/identity.ts` decodes and verifies a Cognito ID token with `node:crypto` (RS256 against the user pool JWKS, cached per issuer for one hour) and checks `alg`, `iss`, `token_use = id`, `aud`, `exp` and `sub`; `loadCognitoAuthConfig` reads the configuration contract `REPOSHERLOCK_COGNITO_USER_POOL_ID`, `REPOSHERLOCK_COGNITO_CLIENT_ID`, `REPOSHERLOCK_COGNITO_REGION` (or `AWS_REGION`), `REPOSHERLOCK_COGNITO_DOMAIN` and `REPOSHERLOCK_COGNITO_REDIRECT_URI`, and reports "not configured" when the deployment has not supplied them. `src/auth/server.ts` implements the authorization-code + PKCE flow: `GET /api/auth/login`, `/api/auth/signup` and `/api/auth/forgot-password` redirect to the hosted UI with a PKCE verifier in an httpOnly cookie, `GET /api/auth/session` completes the `?code=` callback at Cognito's token endpoint and stores the verified ID token in the `reposherlock_session` httpOnly cookie, and `POST /api/auth/logout` clears it. `src/auth/client.ts` is the browser abstraction (`getSession`, `getCurrentUser`, `requireAuth`, `startAuthFlow`, `signOut`, `authHeaders`) and holds only a token Cognito actually issued. `src/api/router.ts` resolves the caller with `requireUser()` (503 `AUTH_NOT_CONFIGURED` when no config, 401 `UNAUTHORIZED` when unauthenticated) and passes that verified id to every repository, index, issue, investigation and evidence operation; a browser-supplied `userId` is never consulted. `src/api/resources.ts` returns 404 for a foreign or unknown repository/investigation, and `src/api/service.ts` stores the creator and only returns investigations to their owner. No AWS resource, CloudFormation template, DynamoDB schema or investigation engine behavior changed.