...
function Issues({ onInvestigate }: { onInvestigate:()=>void }) {
  ...
  return <div className="page-wrap"><div className="page-heading simple"><div><p className="eyebrow">PAYMENTS-SERVICE · 31 OPEN ISSUES</p><h1>What’s the mystery?</h1><p>Pick an issue and let Sherlock investigate it.</p></div></div><div className="tool-row"><div className="search-box"><Search/><Input placeholder="Search issues…"/></div><div className="filters">{["Open","Recently Updated","Bug","Performance","Security","All"].map((f,i)=><Button key={f} variant={i===0?"secondary":"ghost"}>{f}</Button>)}</div></div>
    <div className="issue-list">{issues.map(issue=><button type="button" key={issue.number} className={cn("issue-card",selected===issue.number&&"selected")} onClick={()=>setSelected(issue.number)}><span className="issue-radio">{selected===issue.number&&<i/>}</span><div className="issue-main"><div><span className="issue-number">#{issue.number}</span><span className="status-label"><CircleDot/> {issue.status}</span></div><h3>{issue.title}</h3><div className="labels">{issue.labels.map(l=><span key={l}>{l}</span>)}</div><small>Opened {issue.age} by {issue.author} · {issue.comments} comments</small></div>{selected===issue.number&&<Button onClick={(e)=>{e.stopPropagation();onInvestigate()}}>INVESTIGATE <Search/></Button>}</button>)}</div>
  </div>;
}
